...
url = 'https://api.github.com/repos/microsoft/vscode/pulls?state=all&per_page=100&page='
...
fw = open('vscode_pr', 'wb')
pickle.dump(res, fw)
fw.close()

fw = open('vscode_pr_title.txt', 'w', encoding='utf-8')
for pr in res:
    fw.write(pr['title'] + '\n')
fw.close()

fr = open('vscode_pr', 'rb')
all_pr = pickle.load(fr)
fr.close()

fw = open('pr.json', 'w', encoding='utf-8')
json.dump(all_pr, fw, indent=4, ensure_ascii=False)
fw.close()
res = pd.read_json('pr.json')
res.to_csv('all_pr.csv')
...
fw = open('pr_labels.txt', 'w', encoding='utf-8')
